...
-(id) initWithSVGFilename:(NSString*)filename b2World:(b2World*)world settings:(SVGParserSettings*)sett target:(id)t selector:(SEL)s
{
	if( (self=[super init]) ) {
		...
		NSURL *url = [NSURL fileURLWithPath:[CCFileUtils fullPathFromRelativePath:filename]];
		...
		if(parseError) {
			SVGLOG(@"SVGParser: Error parsing SVG file %@: %@", filename, parseError);
		}
...
-(void) applyPhysicsPropertiesInFixture:(b2FixtureDef*)fixture inBody:(b2Body*)body
{
   ...
   if( twNotEqualFloats(0,density) && bodyType == b2_staticBody )
		bodyType = b2_dynamicBody;
	...
	while( ! finished ) {
		...
		switch( c ) {
			...
			case 'C':
				...
				for( int i=0;i<(int)settings.bezierSegments;i++)
				{
