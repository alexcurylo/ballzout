...
static void readRunInformation(NSArray *attributes, NSUInteger len, CFMutableDictionaryRef fontTableMap,
							   NSUInteger idx, ZAttributeRun **currentRun, NSUInteger *nextRunStart,
							   ZFont **currentFont, fontTable **currentTable) {
	*currentRun = [attributes objectAtIndex:idx];
	*nextRunStart = ([attributes count] > idx+1 ? [[attributes objectAtIndex:idx+1] index] : len);
