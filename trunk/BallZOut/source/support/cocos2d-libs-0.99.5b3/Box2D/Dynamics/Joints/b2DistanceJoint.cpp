...
void b2DistanceJoint::InitVelocityConstraints(const b2TimeStep& step)
{
   ...
   m_mass = twNotEqualFloats(invMass, 0.0f) ? 1.0f / invMass : 0.0f;
	...
	if (m_frequencyHz > 0.0f)
	{
      ...
      // avoid float comparison warning ...alex
      m_gamma = twNotEqualFloats(m_gamma, 0.0f) ? 1.0f / m_gamma : 0.0f;
		m_bias = C * step.dt * k * m_gamma;

		m_mass = invMass + m_gamma;
		//m_mass = m_mass != 0.0f ? 1.0f / m_mass : 0.0f;
      // avoid float comparison warning ...alex
      m_mass = twNotEqualFloats(m_mass, 0.0f) ? 1.0f / m_mass : 0.0f;
	}
