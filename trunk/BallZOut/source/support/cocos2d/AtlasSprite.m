...
-(void)updatePosition
{
	...
	// rotation ? -> update: rotation, scale, position
		...
		float y1c = -transformAnchor.y * scaleY;

		float x2 = x1 + rect_.size.width * scaleX;
		float y2 = y1c + rect_.size.height * scaleY;
		float x = position.x;
		float y = position.y;
		...
		float cr = cosf(r);
		float sr = sinf(r);
		float ax = x1 * cr - y1c * sr + x;
		float ay = x1 * sr + y1c * cr + y;
		float bx = x2 * cr - y1c * sr + x;
		float by = x2 * sr + y1c * cr + y;
	...
	else if(scaleX != 1 || scaleY != 1)
	{
		...
		float y1c = (y- transformAnchor.y * scaleY);
		float x2 = (x1 + rect_.size.width * scaleX);
		float y2 = (y1c + rect_.size.height * scaleY);
		ccQuad3 newVertices = {
			(int)x1,(int)y1c,0,
	...
	// update position
		...
		float y1c = (y-transformAnchor.y);
		float x2 = (x1 + rect_.size.width);
		float y2 = (y1c + rect_.size.height);
		ccQuad3 newVertices = {
			(int)x1,(int)y1c,0,
			(int)x2,(int)y1c,0,
			(int)x1,(int)y2,0,
...
-(void)insertInAtlasAtIndex:(NSUInteger)idx
{
	atlasIndex_ = idx;
