...
{
	float width = self.texture.pixelsWide;
	float height = self.texture.pixelsHigh;
	...
	for( x = 0; x < gridSize.x; x++ )
	{
		for( y = 0; y < gridSize.y; y++ )
		{
			...
			ccVertex3D	e = {x1,y1c,0};
			ccVertex3D	f = {x2,y1c,0};
			...
			CGPoint tex2[4] = { ccp(x1,y1c), ccp(x2,y1c), ccp(x2,y2), ccp(x1,y2) };
...
{
	int	idx = (pos.x * (gridSize.y+1) + pos.y) * 3;
	float *vertArray = (float *)vertices;
	
	ccVertex3D	vert = { vertArray[idx], vertArray[idx+1], vertArray[idx+2] };
...
-(void)setVertex:(ccGridSize)pos vertex:(ccVertex3D)vertex
{
	int	idx = (pos.x * (gridSize.y+1) + pos.y) * 3;
	float *vertArray = (float *)vertices;
	vertArray[idx] = vertex.x;
	vertArray[idx+1] = vertex.y;
	vertArray[idx+2] = vertex.z;
}
