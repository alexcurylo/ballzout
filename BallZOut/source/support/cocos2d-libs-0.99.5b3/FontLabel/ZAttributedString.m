...
- (id)attribute:(NSString *)attributeName atIndex:(NSUInteger)idx effectiveRange:(NSRangePointer)aRange {
	NSParameterAssert(attributeName != nil);
	return [[self attributesAtIndex:idx effectiveRange:aRange uniquingOnName:attributeName] objectForKey:attributeName];
}

- (id)attribute:(NSString *)attributeName atIndex:(NSUInteger)idx longestEffectiveRange:(NSRangePointer)aRange inRange:(NSRange)rangeLimit {
	NSParameterAssert(attributeName != nil);
	return [[self attributesAtIndex:idx longestEffectiveRange:aRange inRange:rangeLimit uniquingOnName:attributeName] objectForKey:attributeName];
}
...
- (NSDictionary *)attributesAtIndex:(NSUInteger)idx effectiveRange:(NSRangePointer)aRange {
	return [NSDictionary dictionaryWithDictionary:[self attributesAtIndex:idx effectiveRange:aRange uniquingOnName:nil]];
}

- (NSDictionary *)attributesAtIndex:(NSUInteger)idx longestEffectiveRange:(NSRangePointer)aRange inRange:(NSRange)rangeLimit {
	return [NSDictionary dictionaryWithDictionary:[self attributesAtIndex:idx longestEffectiveRange:aRange inRange:rangeLimit uniquingOnName:nil]];
}
...
- (NSUInteger)indexOfEffectiveAttributeRunForIndex:(NSUInteger)idx {
	NSUInteger first = 0, last = [_attributes count];
	while (last > first + 1) {
		NSUInteger pivot = (last + first) / 2;
		ZAttributeRun *run = [_attributes objectAtIndex:pivot];
...
- (NSDictionary *)attributesAtIndex:(NSUInteger)idx effectiveRange:(NSRangePointer)aRange uniquingOnName:(NSString *)attributeName {
   (void)attributeName;
	...
	NSUInteger runIndex = [self indexOfEffectiveAttributeRunForIndex:idx];
...
- (NSDictionary *)attributesAtIndex:(NSUInteger)idx longestEffectiveRange:(NSRangePointer)aRange
							inRange:(NSRange)rangeLimit uniquingOnName:(NSString *)attributeName {
	if (idx >= [_buffer length]) {
		@throw [NSException exceptionWithName:NSRangeException reason:@"index beyond range of attributed string" userInfo:nil];
