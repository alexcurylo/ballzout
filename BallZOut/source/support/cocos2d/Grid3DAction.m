...
-(void)update:(ccTime)atTime
{
	...
	for( i = 0; i < (gridSize.x+1); i++ )
	{
		for( j = 0; j < (gridSize.y+1); j++ )
		{
			...
			v.z += (sinf((CGFloat)M_PI*atTime*waves*2 + (v.y+v.x) * .01f) * amplitude * amplitudeRate);
...
-(void)update:(ccTime)atTime
{
	CGFloat angle = (CGFloat)M_PI * atTime; // 180 degrees
	...
	for( i = 0; i < (gridSize.x+1); i++ )
	{
		for( j = 0; j < (gridSize.y+1); j++ )
		{
			...
			if ( r < radius )
			{
				...
				v.z += (sinf( atTime*(CGFloat)M_PI*waves*2 + r * 0.1f) * amplitude * amplitudeRate * rate );
			}
...
-(void)update:(ccTime)atTime
{
	...
	for( i = 1; i < gridSize.x; i++ )
	{
		...
		{
			ccVertex3D	v = [self originalVertex:ccg(i,j)];
			v.x = (v.x + (sinf(atTime*(CGFloat)M_PI*waves*2 + v.x * .01f) * amplitude * amplitudeRate));
			v.y = (v.y + (sinf(atTime*(CGFloat)M_PI*waves*2 + v.y * .01f) * amplitude * amplitudeRate));
			[self setVertex:ccg(i,j) vertex:v];
		}
	}
}	
...
-(void)update:(ccTime)atTime
{
	...
	for( i = 0; i < (gridSize.x+1); i++ )
	{
		for( j = 0; j < (gridSize.y+1); j++ )
		{
			...
			if ( vertical )
				v.x = (v.x + (sinf(atTime*(CGFloat)M_PI*waves*2 + v.y * .01f) * amplitude * amplitudeRate));
			
			if ( horizontal )
				v.y = (v.y + (sinf(atTime*(CGFloat)M_PI*waves*2 + v.x * .01f) * amplitude * amplitudeRate));
...
-(void)update:(ccTime)atTime
{
	...
	for( i = 0; i < (gridSize.x+1); i++ )
	{
		for( j = 0; j < (gridSize.y+1); j++ )
		{
			...
			CGFloat a = r * cosf( (CGFloat)M_PI/2.0f + atTime * (CGFloat)M_PI * twirls * 2 ) * amp;
