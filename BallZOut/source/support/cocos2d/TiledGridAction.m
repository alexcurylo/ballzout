...
-(void)update:(ccTime)atTime
{
	...
	for( i = 0; i < gridSize.x; i++ )
	{
		for( j = 0; j < gridSize.y; j++ )
		{
			tileArray->position = ccpMult( ccp(tileArray->delta.x, tileArray->delta.y), atTime);
...
-(float)testFunc:(ccGridSize)pos time:(ccTime)atTime
{
	CGPoint	n = ccpMult( ccp(gridSize.x,gridSize.y), atTime);
	if ( (n.x+n.y) == 0.0f )
		return 1.0f;
...
-(void)update:(ccTime)atTime
{
	...
	for( i = 0; i < gridSize.x; i++ )
	{
		for( j = 0; j < gridSize.y; j++ )
		{
			float distance = [self testFunc:ccg(i,j) time:atTime];
			if ( distance == 0 )
...
-(float)testFunc:(ccGridSize)pos time:(ccTime)atTime
{
	CGPoint	n = ccpMult(ccp(gridSize.x, gridSize.y), (1.0f-atTime));
...
-(void)update:(ccTime)atTime
{
	...
	for( i = 0; i < gridSize.x; i++ )
	{
		for( j = 0; j < gridSize.y; j++ )
		{
			...
			coords.bl_z = (sinf(atTime*(CGFloat)M_PI*waves*2 + (coords.bl_y+coords.bl_x) * .01f) * amplitude * amplitudeRate );
...
-(void)update:(ccTime)atTime
{
	...
	float sinz =  (sinf((CGFloat)M_PI*atTime*jumps*2) * amplitude * amplitudeRate );
	float sinz2 = (sinf((CGFloat)M_PI*(atTime*jumps*2 + 1)) * amplitude * amplitudeRate );
...
-(void)update:(ccTime)atTime
{
	...
	for( j = 0; j < gridSize.y; j++ )
	{
		...
		coords.bl_x += direction * winSize.width * atTime;
		coords.br_x += direction * winSize.width * atTime;
		coords.tl_x += direction * winSize.width * atTime;
		coords.tr_x += direction * winSize.width * atTime;
...
-(void)update:(ccTime)atTime
{
	...
	for( i = 0; i < gridSize.x; i++ )
	{
		...
		coords.bl_y += direction * winSize.height * atTime;
		coords.br_y += direction * winSize.height * atTime;
		coords.tl_y += direction * winSize.height * atTime;
		coords.tr_y += direction * winSize.height * atTime;
