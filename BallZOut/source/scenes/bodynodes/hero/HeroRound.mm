...
-(void) move:(CGPoint)direction
{
	...
	//
	if( (elapsedTime - lastTimeForceApplied) > kPhysicsHeroForceInterval ) {
		...
		if( controlDirection == kControlDirection2Way ) {
			if( direction.x < 0 )
				f = b2Vec2(-moveForce,0);
			else if(direction.x > 0)
				f = b2Vec2(moveForce,0);
		} else if( controlDirection == kControlDirection4Way )
			f = b2Vec2(direction.x * moveForce, direction.y * moveForce);
		
		
		b2Vec2 p = body_->GetWorldPoint(b2Vec2(0.0f, 0.0f));
		body_->ApplyForce(f, p);				
		
		lastTimeForceApplied = elapsedTime;
...
-(void) jump
{
	...
	if( contactPointCount > 0 ) {
		...
		body_->ApplyLinearImpulse( jumpImpulse * impulseYFactor * normal, p );
	}
	...
	if( ! touchingGround ) {
		...
		if( vel.y > 0 ) {
			...
			float impY = jumpImpulse * vel.y/200;
